...
import { ArrowRight, Plus, Trash2, XIcon } from "lucide-react";
...
export const Links = ({ shopId, links: initialLinks, refetch, isLoading }) => {
  ...
  useEffect(() => {
    const simplifiedLinks = initialLinks.map((link) => ({
      id: link.id, // Assuming 'id' is unique and stable
      name: link.channel ? link.channel.name : "Unnamed", // Simple naming based on channel,
      filtersCount: link.filters?.length,
      rank: link.rank,
    }));
  ...
  return (
    <div className="mt-3 grid grid-cols-2 gap-4">
      ...
      <div className="flex flex-col gap-3">
        <div className="p-3 bg-muted text-muted-foreground rounded-lg border flex flex-col">
...
export const Link = ({ link, isSelected, onSelect, onDelete }) => {
  return (
      ...
      <div className="flex items-center justify-between w-full">
        ...
        <div className="flex items-center gap-1">
          ...
          <Badge
            ...
            onClick={onSelect}
          >
            {link.filtersCount || 0} <ArrowRight className="w-3 h-3" />
          </Badge>
        </div>
      </div>
    </div>
