...
export const CartItemSelect: React.FC<CartItemSelectProps> = ({
  ...
  const handleAddItem = async (product: Product) => {
    ...
    try {
      ...
      // If onSelectionChange is provided, manage selectedItems locally (OrderDetailsDialog mode)
      ...
      // Otherwise use the legacy onAddItem callback (other components)
      else if (onAddItem && orderId) {
  ...
  return (
    ...
    >
      ...
      <CollapsibleContent className="space-y-2">
        {/* Display existing selected cart items - ABOVE search bar */}
        {selectedItems.length > 0 && (
          <div className="space-y-2">
            <h4 className="text-sm font-medium text-emerald-600 dark:text-emerald-400">Selected Cart Items ({selectedItems.length})</h4>
