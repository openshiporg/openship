...
export const CartItemSelect: React.FC<CartItemSelectProps> = ({
  ...
  const handleAddItem = async (product: Product) => {
    ...
    try {
      ...
      let updatedItems: SelectedCartItem[];
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-[2] min-h-0 mb-4">
        {selectedItems.length > 0 ? (
          <div className="bg-emerald-50/30 dark:bg-emerald-900/10 p-4 rounded-lg border border-emerald-200 dark:border-emerald-900 h-full flex flex-col">
            <h4 className="text-sm font-medium uppercase tracking-wide mb-3 text-emerald-500 dark:text-emerald-300 flex-shrink-0">
                ...
                >
                  <div className="flex items-start space-x-3 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium mb-1">{item.channel.name}</p>
          ...
          <div className="bg-emerald-50/30 dark:bg-emerald-900/10 p-4 rounded-lg border border-emerald-200 dark:border-emerald-900 h-full flex items-center justify-center">
            <p className="text-sm text-muted-foreground">No cart items selected</p>
          </div>
        )}
      </div>
        ...
        <div className="flex space-x-2">
          <Select value={selectedChannelId} onValueChange={setSelectedChannelId} disabled={channels.length === 0}>
            <SelectTrigger className="w-auto min-w-[200px] h-9">
              <SelectValue placeholder={channels.length === 0 ? "No channels available" : "Select Channel"} />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="flex-[2] min-h-0 mt-4">
        {searchResults.length > 0 ? (
          ...
          <div className="bg-foreground/5 p-4 rounded-lg border h-full flex items-center justify-center">
            {!loading && searchEntry ? (
              <p className="text-sm text-muted-foreground">No products found for this channel.</p>
            ) : (
