...
export function getDocumentFeaturesForChildField(editorDocumentFeatures, options) {
  ...
  const inlineMarks =
    inlineMarksFromOptions === 'inherit'
      ? 'inherit'
      : (Object.fromEntries(Object.keys(editorDocumentFeatures.formatting.inlineMarks).map(mark => {
...
export function replaceValueAtPropPath(schema, value, newValue, path) {
  ...
  assert(
    schema.kind !== 'form' && schema.kind !== 'relationship' && schema.kind !== 'child'
  )
