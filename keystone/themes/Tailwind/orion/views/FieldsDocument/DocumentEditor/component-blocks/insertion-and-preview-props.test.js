...
const getPreviewProps = editor =>
  createGetPreviewProps(
    { kind: "object", fields: componentBlocks.complex.schema },
    props => {
      Transforms.setNodes(
        editor,
        { props: props(editor.children[0].props) },
        { at: [0] }
      )
    },
    path => React.createElement(ChildFieldEditable, { path })
  )(editor.children[0].props)
...
test("preview props conditional change", () => {
  ...
  `)
  const conditionalPreviewProps = getPreviewProps(editor).fields.object.fields
    .conditional
  const expectedConditionalPreviewProps = {
    schema: componentBlocks.complex.schema.object.fields.conditional,
    discriminant: true,
    onChange: expect.any(Function),
    options: undefined,
...
test("preview props form change", () => {
  ...
  expect(editor.children[0].props.object.select).toBe("b")
...
test("relationship many change", () => {
  ...
  expect(editor.children[0].props.object.many).toEqual(val)
...
test("relationship single change", () => {
  ...
  expect(editor.children[0].props.object.conditional.value).toEqual(val)
  expect(
    getPreviewProps(editor).fields.object.fields.conditional.value.value
  ).toEqual(val)
})
...
test("changing conditional with form inside", () => {
  ...
  expect(editor.children[0].props.object.conditionalSelect)
  ...
  `)
  const conditionalSelectPreviewProps = getPreviewProps(editor).fields.object
    .fields.conditionalSelect
  const expectedConditionalSelectPreviewProps = {
    discriminant: "b",
    ...
    value: {
      schema:
        componentBlocks.complex.schema.object.fields.conditionalSelect.values.b,
      onChange: expect.any(Function),
      options: undefined,
      value: "B"
    }
...
test("changing form inside conditional", () => {
  ...
  const expectedConditionalSelectPreviewProps = {
    ...
    value: {
      schema:
        componentBlocks.complex.schema.object.fields.conditionalSelect.values.a,
      onChange: expect.any(Function),
      options: undefined,
      value: "Some content"
    }
