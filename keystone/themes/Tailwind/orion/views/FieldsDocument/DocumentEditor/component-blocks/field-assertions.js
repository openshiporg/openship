...
export function assertValidComponentSchema (schema, lists) {
...
function assertValidComponentSchemaInner (
  ...
  if (schema.kind === 'relationship') {
    ...
    throw new Error(`The relationship field at "${propPath.join('.')}" has the listKey "${
      schema.listKey
  ...
  if (ancestor !== -1) {
    throw new Error(
      `The field "${propPath.join('.')}" is the same as it's ancestor. Use an array or conditional field for recursive structures.`
    )
  }
    ...
    if (schema.kind === 'object') {
      ...
      for (const [key, innerProp] of Object.entries(schema.fields)) {
        ...
        if (schema.fields[key] !== innerProp) {
          throw new Error(
            `Fields on an object field must not change over time but the field at "${propPath.join('.')}" changes between accesses`
          )
        }
    ...
    if (schema.kind === 'conditional') {
      ...
      for (const [key, innerProp] of Object.entries(schema.values)) {
        ...
        if (schema.values[key] !== innerProp) {
          throw new Error(
            `Fields on a conditional field must not change over time but the field at "${propPath.join('.')}" changes between accesses`
          )
        }
