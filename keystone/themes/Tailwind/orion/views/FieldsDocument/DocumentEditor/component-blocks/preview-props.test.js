...
test(
  "onChange on a conditional field updates props.value where the value doesn't change",
  () => {
    const field = fields.conditional(fields.checkbox({ label: '' }), {
...
test(
  "array fields don't keep the props of an array item around after removing the item",
  () => {
...
test(
  'the props of two array fields in a conditional field change when the discriminant changes',
  () => {
    const field = fields.conditional(fields.checkbox({ label: '' }), {
